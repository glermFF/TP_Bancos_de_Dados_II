...
export function Field({ label, error, hint, style, ...inputProps }: Props) {
  ...
  return (
    <View style={styles.wrap}>
      ...
      <TextInput
        placeholderTextColor="rgba(20,20,20,.40)"
        onFocus={() => setFocused(true)}
        onBlur={() => setFocused(false)}
        style={[styles.input, focused && styles.inputFocused, !!error && styles.inputError, inputProps.multiline && styles.inputMultiline, style]}
        {...inputProps}
      />
...
const styles = StyleSheet.create({
  ...
  input: { fontFamily: fonts.serif, fontSize: 16, color: colors.ink, backgroundColor: colors.paper, borderWidth: 1, borderColor: colors.rule, borderRadius: 4, paddingVertical: 12, paddingHorizontal: 14 },
