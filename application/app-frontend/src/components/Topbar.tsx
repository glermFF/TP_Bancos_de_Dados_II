  ...
  return (
    <View style={styles.bar}>
      <Hoverable style={styles.brand} onPress={() => nav.navigate('Home')}>
        <Image source={require('../../assets/logo-mg.png')} style={styles.mark} resizeMode="contain" />
        <View>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Hoverable key={item.name} onPress={() => nav.navigate(item.name as never)}>
              {(hovered: boolean) => (
                <Text style={[styles.navLink, on && styles.navLinkActive, hovered && !on && styles.navLinkHover]}>
      ...
      <View style={styles.actions}>
        {user ? (
          ...
          <Hoverable style={styles.signIn} hoverStyle={styles.signInHover} onPress={() => nav.navigate('SignIn')}>
            {(hovered: boolean) => (
...
const styles = StyleSheet.create({
  bar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', rowGap: 14, columnGap: 18, paddingTop: 8, paddingBottom: 18, borderBottomWidth: 1, borderBottomColor: colors.rule },
  ...
  navLink: { fontFamily: fonts.mono, fontSize: 11, letterSpacing: letterSpacing.label, color: colors.inkSoft, paddingBottom: 3, borderBottomWidth: 1, borderBottomColor: 'transparent' },
  ...
  signIn: { borderWidth: 1, borderColor: colors.ink, borderRadius: 999, paddingVertical: 9, paddingHorizontal: 18, backgroundColor: 'transparent' },
  ...
  userChip: { flexDirection: 'row', alignItems: 'center', columnGap: 8, borderWidth: 1, borderColor: colors.rule, borderRadius: 999, paddingVertical: 8, paddingHorizontal: 14, backgroundColor: colors.paper },
